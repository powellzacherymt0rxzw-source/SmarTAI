...
async def prepare_question_packages(
    ...
    *,
    ...
    The returned mapping is not persisted here.  The API worker commits it via
    TaskStore only after every substep succeeds, preserving the previous good
    version if a later material step fails.
    """
    ...
    for (q_id, target), rows in selected_candidates.items():
        ...
        provenance[target] = {
            "import_job_id": "unified_question_preparation",
    ...
    if requested_targets:
        ...
        for index, candidate in enumerate(generated, start=1):
            ...
            provenance[candidate.target] = {
                "job_id": "unified_question_preparation",
