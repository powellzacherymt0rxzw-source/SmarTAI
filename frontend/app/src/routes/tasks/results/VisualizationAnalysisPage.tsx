...
export function VisualizationAnalysisPage({ locale, taskId, version, model }: { locale: Locale; taskId: string; version: number; model: ResultsModel }) {
  ...
  const [prompt, setPrompt] = useState(tx(locale, "画出学生得分率与平均置信度的关系，并区分需要复核的学生。", "Plot score rate against mean confidence and distinguish students needing review."));
  ...
  return (
    <section className="rounded-[10px] border bg-card">
      ...
      <div className="mt-4 grid gap-4 border-t p-5 xl:grid-cols-2">
        ...
        <ChartCard locale={locale} id="pass-composition" title={tx(locale, "及格构成", "Pass composition")} description={tx(locale, "只比较互斥的及格、未及格与无可比总分，不把复核状态混入同一饼图。", "Compares mutually exclusive passed, failed, and unscored groups; review state is kept separate.")} metadata={chartMetadata(locale, students.length, version, scope)} detailHref={`${root}/students`}>
        ...
        <ChartCard locale={locale} id="question-performance" title={tx(locale, "逐题得分率与复核量", "Question score rate & review volume")} description={tx(locale, "柱表示平均得分率，折线表示必审题次数，用于同时发现薄弱题与证据风险。", "Bars show mean score rate; the line shows required-review volume to expose both weak questions and evidence risk.")} metadata={chartMetadata(locale, students.length, version, scope)} detailHref={`${root}/questions`} wide>
          <div style={{ width: `${Math.max(620, questionData.length * 72)}px`, height: 280 }}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={questionData} margin={{ top: 14, right: 18, left: -10, bottom: 4 }}><CartesianGrid stroke={COLORS.grid} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" tick={{ fontSize: 10 }} /><YAxis yAxisId="score" domain={[0, 100]} tick={{ fontSize: 10 }} unit="%" /><YAxis yAxisId="review" orientation="right" allowDecimals={false} tick={{ fontSize: 10 }} /><Tooltip contentStyle={tooltipStyle} /><Legend wrapperStyle={{ fontSize: 11 }} /><Bar yAxisId="score" dataKey="scoreRate" name={tx(locale, "平均得分率", "Mean score rate")} fill={COLORS.teal} radius={[5, 5, 0, 0]} /><Line yAxisId="review" type="monotone" dataKey="reviewCount" name={tx(locale, "必审题次", "Review items")} stroke={COLORS.rose} strokeWidth={2} dot={{ r: 3 }} /></ComposedChart></ResponsiveContainer></div>
        </ChartCard>

        <ChartCard locale={locale} id="confidence-scatter" title={tx(locale, "得分率 × 平均置信度", "Score rate × mean confidence")} description={tx(locale, "每个点是一位学生；红色只表示含正式必审信号，便于识别高分低置信等异常组合。", "Each point is a student; red marks formal review signals, exposing combinations such as high score with low confidence.")} metadata={chartMetadata(locale, students.length, version, scope)} detailHref={`${root}/students`} wide>
          <ResponsiveContainer width="100%" height={280}><ScatterChart margin={{ top: 16, right: 18, left: -4, bottom: 4 }}><CartesianGrid stroke={COLORS.grid} strokeDasharray="3 3" /><XAxis type="number" dataKey="score" name={tx(locale, "得分率", "Score rate")} domain={[0, 100]} unit="%" tick={{ fontSize: 10 }} /><YAxis type="number" dataKey="confidence" name={tx(locale, "平均置信度", "Mean confidence")} domain={[0, 100]} unit="%" tick={{ fontSize: 10 }} /><Tooltip cursor={{ strokeDasharray: "3 3" }} contentStyle={tooltipStyle} /><Legend wrapperStyle={{ fontSize: 11 }} /><Scatter name={tx(locale, "无必审项", "No required review")} data={scatterData.filter((item) => !item.review)} fill={COLORS.primary} /><Scatter name={tx(locale, "含必审信号", "With review signals")} data={scatterData.filter((item) => item.review)} fill={COLORS.rose} /></ScatterChart></ResponsiveContainer>
        </ChartCard>

        <ChartCard locale={locale} id="score-heatmap" title={tx(locale, "学生 × 题目得分热力图", "Student × question score heatmap")} description={tx(locale, "颜色表达逐格得分率，单元格保留真实百分比；适合寻找个人薄弱题和班级共性。", "Cell color encodes score rate while retaining the actual percentage, revealing individual and class-wide weaknesses.")} metadata={chartMetadata(locale, Math.min(students.length, 100), version, scope, students.length > 100 ? tx(locale, "最多显示前 100 位", "First 100 students") : "")} detailHref={`${root}/students`} wide>
      ...
      <div className="border-t p-5">
        <section className="rounded-[9px] border px-4 py-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="flex items-center gap-2 text-[15px] font-bold text-foreground"><Sparkles aria-hidden="true" className="h-4 w-4 text-primary" />{tx(locale, "自然语言生成更多图表", "Generate another chart with natural language")}</h3><p className="mt-1 text-[11px] text-muted-foreground">{tx(locale, "提交会调用当前配置模型一次；后端仅接受 bar / scatter / pie / histogram / box，最多 4 条 trace、每条 50 个点。", "Submission makes one configured-model call; the backend accepts only bar / scatter / pie / histogram / box, up to 4 traces and 50 points each.")}</p></div><span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-semibold text-amber-700">{tx(locale, "不会自动调用", "Never automatic")}</span></div>
        ...
        {savedCharts.length ? <section className="mt-4"><div className="flex items-end justify-between gap-3"><div><h3 className="text-[15px] font-bold text-foreground">{tx(locale, "本次浏览已保存", "Saved for this visit")}</h3><p className="mt-1 text-[10px] text-muted-foreground">{tx(locale, "当前仅保存在页面内存，刷新后清空；后端持久化与版本历史仍待接入。", "Currently held only in page memory and cleared on refresh; server persistence and version history remain pending.")}</p></div><span className="text-[10px] text-muted-foreground">{savedCharts.length}</span></div><div className="mt-3 grid gap-4 xl:grid-cols-2">{savedCharts.map((item) => <GeneratedResult key={item.id} locale={locale} id={item.id} result={item.result} version={version} prompt={item.prompt} onDelete={() => setSavedCharts((items) => items.filter((candidate) => candidate.id !== item.id))} />)}</div></section> : null}
      </div>
    </section>
  );
}
...
function GeneratedResult({ locale, id, result, version, prompt, onSave, onDelete }: { locale: Locale; id: string; result: ChartAnalyticsResult; version: number; prompt?: string; onSave?: () => void; onDelete?: () => void }) {
  ...
  return <article className="mt-3 rounded-[9px] border px-4 py-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="text-[13px] font-bold text-foreground">{result.title || tx(locale, "自定义图表", "Custom chart")}</h4><p className="mt-1 text-[10px] leading-4 text-muted-foreground">{result.rationale || prompt}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => void exportChartPng(id, result.title, locale)} className="inline-flex h-8 items-center gap-1 rounded-[7px] border px-2 text-[10px] font-semibold"><Download aria-hidden="true" className="h-3.5 w-3.5" />{traces.length > 1 ? tx(locale, "首图 PNG", "First PNG") : "PNG"}</button>{onSave ? <button type="button" onClick={onSave} className="inline-flex h-8 items-center gap-1 rounded-[7px] bg-primary px-2.5 text-[10px] font-semibold text-primary-foreground"><Save aria-hidden="true" className="h-3.5 w-3.5" />{tx(locale, "保存本次", "Save")}</button> : null}{onDelete ? <button type="button" onClick={onDelete} aria-label={tx(locale, "删除本次保存图表", "Delete saved chart")} className="inline-flex h-8 w-8 items-center justify-center rounded-[7px] border text-rose-600 hover:bg-rose-50"><Trash2 aria-hidden="true" className="h-3.5 w-3.5" /></button> : null}</div></div><div className="mt-2 flex flex-wrap gap-1.5"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] text-muted-foreground">v{version}</span><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] text-muted-foreground">{traces.length} trace</span></div><div data-chart-export={id} className="mt-3 grid gap-3 xl:grid-cols-2">{traces.length ? traces.map((trace, index) => <GeneratedTrace key={`${trace.type}-${index}`} locale={locale} trace={trace} index={index} />) : <p className="col-span-full py-6 text-center text-[11px] text-muted-foreground">{tx(locale, "没有可安全渲染的 trace。", "No trace can be rendered safely.")}</p>}</div></article>;
}
...
function buildQuestionPerformance(questions: QuestionSummary[], studentIds: Set<string>) { return questions.map((question) => { const entries = question.entries.filter((entry) => studentIds.has(entry.student.id)); const percents = entries.map((entry) => entry.correction.max_score > 0 ? (effectiveCorrectionScore(entry.correction) / entry.correction.max_score) * 100 : null).filter((value): value is number => value !== null); return { label: question.label, scoreRate: averageOrNull(percents) ?? 0, reviewCount: entries.filter((entry) => correctionNeedsFormalReview(entry.correction)).length }; }); }
...
function chartMetadata(locale: Locale, sample: number, version: number, scope: ScopeFilter, extra = ""): string[] { return [tx(locale, `样本 ${sample}`, `Sample ${sample}`), tx(locale, `正式结果 v${version}`, `Formal result v${version}`), scopeLabel(locale, scope), extra].filter(Boolean); }
