  ...
  return (
    <section className="rounded-[10px] border bg-card">
      ...
      <div className="mt-4 grid gap-4 border-t p-5 xl:grid-cols-2">
        ...
        <ChartCard locale={locale} id="question-performance" title={tx(locale, "逐题得分率与复核量", "Question Score Percentage & Review Volume")} description={tx(locale, "柱表示平均得分率，折线表示复核信号题次数，用于同时发现薄弱题与证据风险。", "Bars show the average score percentage; the line shows review-signal volume, highlighting both difficult questions and evidence risks.")} metadata={chartMetadata(locale, students.length, version, scope)} detailHref={`${root}/questions`} wide>
