...
const copy = {
  ...
  recoverableHint: ["如果出现 API 超量或网络超时，这里会显示真实错误、重试和修改专家入口。", "If an API quota or network timeout occurs, this area will show the real error, retry, and expert settings actions."],
  failedTitle: ["本次批改没有完成", "This grading run did not finish"],
  failedDescription: ["任务数据仍然保留。可以重试同一任务，或先修改任务级专家组合。", "Task data is preserved. Retry this task or adjust its task-level expert selection first."],
  retry: ["重试批改", "Retry grading"],
  retrying: ["正在重试…", "Retrying…"],
  editExperts: ["修改专家组合", "Edit expert selection"],
