...
- [~] Q03 题目准备总览改为风险优先矩阵。工程与浏览器验收完成：2026-07-26；正常识别/生成数据不显示“缺失”，只列 open risk；旧“批量导入资料/AI 补全缺失项”入口已删除；独占搜索支持 IME，表头支持双向排序，题型列支持多选。等待用户视觉确认。
...
- 顶部流程统一为八步：`新建任务 / 上传题目 / 审核题目 / 校对作答 / 批改设置 / 执行批改 / 复核分析 / 完成`；“上传题目”是最终命名。当前和历史步骤均为真实链接，未来步骤为 disabled。
