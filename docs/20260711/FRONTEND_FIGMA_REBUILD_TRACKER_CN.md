...
- 顶部流程最终统一为八步：`新建任务 / 上传题目 / 审核题目 / 上传作答 / 校对作答 / 执行批改 / 复核分析 / 完成`；“上传题目”是最终命名，批改/识别设置合并到“上传作答”的内部第二阶段。任务持久状态决定最远可达步骤：返回早期页面不会再次锁住已完成阶段，未来阶段仍 disabled。
...
- [~] S04/S05 已合并为一套学生作答连续校对页。canonical route 为 `/tasks/:id/students/:sid?question=:qid`；单学生指标、身份修正、学生直选、题目搜索、左侧独立滚动题号栏、全部题目连续渲染、LaTeX 浏览/源码编辑、键盘导航和真实答案 CAS 均保留。2026-07-27 增加异常学生着色、逐题持久“已复核”和修改后自动重开；旧 `/questions/:qid` route 与旧总览组件已删除，等待用户视觉确认。
