...
- 页面职责：canonical route 为 `/tasks/:id/submissions/upload`；内部先完成“选择作答来源/身份匹配”，再进入“识别设置”。设置保存后立即开始识别并进入 `/submissions/progress`；`already_done` 与识别完成状态统一进入 `/submissions`。
- 真实能力：支持可复制文字 PDF、TXT/MD/RST/CSV，以及 ZIP/RAR/7z/TAR 压缩包；图片、扫描件、手写 OCR 未实现，页面明确显示“OCR 增强：暂不可用”，没有照抄 Figma 而虚构支持。
- 身份与隐私：文件名优先、名单确定性匹配、全部人工复核三种模式均进入真实后端合同；名单只在服务端精确匹配，整份名单不发送给模型，未唯一匹配者进入人工复核。识别 provider 必须归当前用户、已启用，并使用同页刚保存的任务 primary provider。
