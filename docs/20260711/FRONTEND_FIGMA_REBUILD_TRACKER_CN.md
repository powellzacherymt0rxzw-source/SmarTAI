...
>
> 当前阶段：K00 已完成 Figma 09 所需的真实课程资料库后端合同：文件上传/去重、分组、课程、标签、确定性查找、owner 隔离和任务引用删除保护均通过测试；下一步 K01 只按 Figma 09 从头替换 `/knowledge-base` 可见页面。`auto-research-stateprep-v2` 已完成并空闲；A07 后官方只读接口显示周额度剩余约 26%，43% 起点至当时 12 个阶段平均约 1.4 个百分点/阶段，继续以 15%+ 为硬保留线。既有等待确认状态不变。
...
- [~] G04-01 课程资料库使用 Figma 09 结构，支持真实文件、资料分组、标签、搜索和跨任务选择。后端 K00 完成：2026-07-24；`/course-materials/*` 已提供真实解析上传、去重、分组/标签/课程、确定性查找、owner 隔离、任务引用与删除保护，Q01/Q08 已记录跨任务引用；合并回归 `25 passed`。可见 Figma 09 页面与双尺寸截图留给 K01，故保持 `[~]`。
- [ ] G05-01 账户设置从用户名菜单进入。
