...
- [~] Q03 题目准备总览只做覆盖率、筛选、排序和分流。工程完成并按用户纠正：2026-07-20；四个覆盖率指标借鉴 Figma 14 的大号圆角统计卡独占一行，搜索框另起整行，排序降为辅助控件，矩阵保持独立卡和内部滚动。`1440×900` 四题数据首屏无页面滚动，`390×844` 无页面级横向溢出；组合筛选与 URL/返回状态未改变，等待用户视觉确认。
