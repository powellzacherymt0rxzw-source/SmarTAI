...
>
> 当前阶段：R6-01/02 全局页面专注度与信息密度工程审计已完成。canonical route 已按“总览分流、详情处理”拆分；旧长页只保留状态感知跳转，用户数据本身以外不制造长滚动。69 文件 lint/TypeScript 通过，并新增用户可见开发者术语防回归。现进入 R6-03 至 R6-07 合并式最终回归；不重复已有 Figma 双尺寸截图，R6-08 仍由用户逐页验收。
