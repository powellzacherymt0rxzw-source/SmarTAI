...
| D-14 | Q01 同时准备题目、标答、评分标准和编程题测试样例 | 已确认 | 题目必选，其他三类可选；每份资料独立选择上传/资料库和按题/原文提取。 |
| D-15 | 一次 AI 任务识别全部来源并补齐未上传资料 | 已确认 | 仅给最终答案时生成过程；标答步骤与评分步骤建立对应；前端只出现一次主要运行流程。 |
...
### 3.6 题目资料统一准备与连续审核（2026-07-26 覆盖）

详细模型、API、状态机、迁移和验收矩阵以
...
- [ ] BE-PREP-06 新增 compact 题号 index、游标加载题目包、风险查询、字段级 CAS PATCH 和页面底部 confirm-all；“保存并下一题”只保存后返回相邻锚点，不建立另一套单题模式。
- [ ] BE-PREP-07 状态机增加 `preparing_questions → questions_review_pending → problems_ready` 语义；统一 Job 完成不等于教师确认，blocking issue 未解决时 confirm-all 必须拒绝。
- [ ] BE-PREP-08 manifest fingerprint 包含所有来源 hash/role/mode/hint、确认候选、provider/model 与 prompt/schema version；覆盖 same-running、same-done、different-running、失败重试、worker 过期和 CAS，失败保留上一成功版本。
- [ ] BE-PREP-09 题目准备阶段支持标答、评分标准和编程测试真实导出；hidden test 只能进入显式教师包，学生接口和公开导出只能返回数量。
- [ ] BE-PREP-10 旧 `/extract_problems`、`/upload_reference`、`/upload_test_cases`、Q08、Q09 路由迁入同一 domain service 后再逐步废弃；不得保留两套编排逻辑。
- [ ] BE-PREP-11 先定义来源、Job、题目包和 issue repository protocol；内存 adapter 只作过渡，正式 PostgreSQL/对象存储接入时不重写 Agent/Skill/API 语义。
...
- [~] G03-10 创建后可返回并编辑当前任务名称、学期、课程和标签。工程与浏览器验收完成：2026-07-26；新增 `/tasks/:id/edit` 复用同一 Figma 03 表单，Q01 返回草稿保留，服务端拒绝空白名称并继续 owner 校验；等待用户视觉确认。
...
### 5.4.1 G07 当前任务信息与 BYOK 门禁纠正记录（2026-07-26）

- 详细记录：`docs/20260726/G07_TASK_METADATA_AND_BYOK_CORRECTION_STAGE_CN.md`。
- 创建后的 Q01 标题右侧新增“编辑任务信息”，返回原 Q01 时保留文件、资料库选择、结构模式、提取说明和保存选项；保存继续使用 owner-scoped `PUT /tasks/:id`。
- 多选标签中新建值完成后弹层立即关闭；选择已有多标签时仍允许连续选择。
- 0/0 模型时“开始识别题目”保持灰态但可点击说明；对话框提供携带当前 Q01 草稿的真实 BYOK 入口。文件缺失/请求处理中等其他门禁仍是真实 disabled。
- 工程证据：69 文件 visible-scope/lint/TypeScript、Vite `925 modules`、New Task 合同 `8 passed` 和 `git diff --check` 通过。
- 浏览器证据：新标签后 `aria-expanded=false`；真实创建→Q01→编辑回填→保存返回闭环及 0/0 BYOK 对话框通过。截图位于临时可视化目录 `20260726-stage1/`，零 provider。
- 最新覆盖：用户同日确认 Q01 将承担题目/标答/评分标准/测试样例的统一来源准备和一次 AI 识别；Q08/Q09 不再作为 Q03 下方独立补缺主流程。该新阶段尚未实现，不能计入 G07 完成范围。
...
- [~] G00-01 登录页后置品牌视觉，不影响主流程优先级。工程与浏览器验收完成：2026-07-24；登录、邀请注册、学生端未开放和会话恢复共用克制的 Figma 同系认证壳，中英文、错误登录、无效邀请码、受保护 route 回跳及 390px 无溢出均通过；邀请已补齐邮箱绑定、过期和原子一次性消费。等待用户视觉确认，故保持 `[~]`。
...
### 5.6 G-00 登录与邀请注册阶段工程与自动验收记录（2026-07-24）

- 页面结构：无独立 Figma 帧；使用 440px 居中单卡、44px 控件、12px 圆角、浅蓝灰背景和低对比静态教育线稿，不引入营销分栏、渐变、Emoji 或动画。
- 功能证据：真实教师登录、同源受保护 route 回跳、教师角色门禁、邀请制注册、学生端边界、密码显隐及中英文状态已接入；注册不再是假按钮。
- 邀请证据：邀请码大小写无关且一次性原子消费；指定邮箱不匹配不烧码，过期码移除，只含空格用户名在消费前拒绝。
- 工程证据：85 个可见文件 scope audit、lint/TypeScript、Vite `953 modules` build、定向 `4 passed`、全量后端 `229 passed, 1 skipped` 和 `git diff --check` 通过。
- 浏览器证据：中英文切换、错误登录、从 `/history` 登录后回跳、无效邀请码错误以及 `1440×900`/`390×844` 通过；移动 `scrollWidth=clientWidth=390`。
...
- [~] Q01-09 将来源工作区扩为题目、标答、评分标准、编程题测试样例四个资料位；题目必选，其余可选；支持左右切换和连续查看。工程与浏览器验收完成：2026-07-26；每类可添加多份独立来源，等待用户视觉确认。
- [~] Q01-10 每份资料独立选择上传/课程资料库、已按题整理/从原文提取及提取说明；不得继续让多类资料共用一个来源设置。工程完成：role-bound token、每份来源独立配置和多来源统一请求已接入；同字段多来源差异产生 open risk，不按上传顺序静默视为无冲突。结构化 alternatives 持久化仍按后端 BQ-03 实施。
- [~] Q01-11 主动作改为“识别并准备题目资料”，一个编排处理所有上传来源并生成未上传的标答/评分标准；只含最终答案时补全过程，输出答案步骤与评分步骤对应关系。工程完成边界：统一 job、答案过程扩展和 rubric 对齐 prompt 已接入；稳定 step ID/引用校验仍待结构化后端模型。
- [~] Q01-12 后端为每题保存题干/标答/评分标准的来源、置信度、文件冲突与解析异常；正常生成完成不进入缺失补录路径。兼容期工程完成：provenance、置信度和 open issue 可供前端消费，正常 AI 生成不显示 missing；字段级版本与完整 issue repository 仍待后端阶段。
- [~] Q01-13 0/0 模型门禁可解释。工程与浏览器验收完成：2026-07-26；灰态 CTA 悬浮解释、点击对话框和携带草稿的 BYOK 入口已接入，等待用户视觉确认。

2026-07-26 最新覆盖已按兼容期合同落地。Q01-01 至 Q01-08 的安全合同继续复用；Q01-09 至 Q01-12 的前端和统一 job 首版已完成工程验收，但结构化题目包、字段级 CAS 与持久化仍须以后端契约第 14 节为准，等待用户视觉确认前保持 `[~]`。
...
- [~] Q09 AI 补全底层能力迁入 Q01 一次准备编排。原 missing-only/CAS/provenance/后台恢复合同可复用；2026-07-26 起不再显示独立“AI 补全缺失项”按钮，未上传标答/评分标准在题目识别同一任务中生成并一起进入审核。
...
### 6.0A 2026-07-26 统一准备与连续审核工程记录

- 顶部流程最终统一为八步：`新建任务 / 上传题目 / 审核题目 / 上传作答 / 校对作答 / 执行批改 / 复核批改 / 结果分析`；“上传题目”是最终命名，作答识别设置留在“上传作答”，批改配置归入第 6 步。任务持久状态决定最远可达步骤：返回早期页面不会再次锁住已完成阶段，未来阶段仍 disabled。
- Q01、Q03 与连续审核的页面、后端兼容期合同、已知边界和验收证据集中记录于 `docs/20260726/Q01_Q03_UNIFIED_PREPARATION_IMPLEMENTATION_STAGE_CN.md`。
- 后端唯一后续安排已写入 `docs/20260726/Q01_Q03_UNIFIED_QUESTION_PREPARATION_BACKEND_PLAN_CN.md` 第 14 节，明确区分已实现兼容层与未实现结构化模型/持久化/沙箱，便于后续一次迁移。
- 工程证据：前端 69 文件 visible-scope、lint、TypeScript 和 Vite build（`929 modules`）通过；后端定向/兼容回归 `51 passed, 1 skipped`，整库 `233 passed, 1 skipped`；Python 编译与 `git diff --check` 通过。
- 浏览器证据：八步回链、四资料位、0/0 BYOK 说明、风险排序/多选筛选、左侧题号栏、连续题目包、非编程条件隐藏、OJ 样例、KaTeX 浏览态和源码编辑态均通过，最终控制台 0 error/0 warning。
- Figma 06/07 与实现已生成同屏对照，项目根目录 `design-qa.md` 结论为 `passed`；按 tracker 规则仍等待用户主观视觉确认，故相关项保持 `[~]`。
...
- [~] S01 上传作答只负责“选择文件 / 身份匹配 → 开始识别作答”。工程再纠偏：2026-07-27；删除内嵌批改设置阶段，三种身份匹配、替换确认、BYOK 恢复和识别进度保留。识别未显式指定 provider 时使用当前用户默认可用模型，不再消费历史批改设置。等待用户视觉确认。
...
- Figma 约束：使用 Figma 10（文件 `64TupCQCKXkiT5uxeQY0iH`，节点 `1:679`）作为唯一可见基线；桌面保持标题、`900×230` 上传区、约 `900×145` 身份区和单一 CTA，没有复用遗留上传长页，也不在此页提前展示批改设置。
- 页面职责：canonical route 为 `/tasks/:id/submissions/upload`；只完成“选择作答来源 / 身份匹配 → 开始识别作答”，随后进入 `/submissions/progress`；`already_done` 与识别完成状态统一进入 `/submissions`。批改设置只在第 6 步编辑。
- 真实能力：支持可复制文字 PDF、TXT/MD/RST/CSV，以及 ZIP/RAR/7z/TAR 压缩包；图片、扫描件、手写 OCR 未实现，页面明确显示“OCR 增强：暂不可用”，没有照抄 Figma 而虚构支持。
- 身份与隐私：文件名优先、名单确定性匹配、全部人工复核三种模式均进入真实后端合同；名单只在服务端精确匹配，整份名单不发送给模型，未唯一匹配者进入人工复核。识别 provider 必须归当前用户且已启用；未显式指定时使用当前用户默认可用 provider，不从批改设置暗中继承。
...
- 页面职责（2026-07-27 最新覆盖）：canonical route `/tasks/:id/review/:sid/:qid` 始终展示当前学生全部筛选题目的连续复核工作区；`:qid` 只负责首次定位，不再维护“单题 / 全部”两套模式。左侧题号栏、题目搜索、题卡上下题按钮和正文滚动同步。
- 智能与键盘：学生/题目筛选分别保留在 URL；exact/related 明示；“积分题”和完整学号浏览器实测通过；左右切学生、上下切题目，编辑控件聚焦不截获。
- 结果布局（2026-07-27 最新覆盖）：“AI 判定与依据”改为“SmarTAI 批改结果”，完整显示理由和结构化步骤；教师最终得分放在右上角并以 `x / x` 呈现，评语独占下方编辑区。
- 真实合同：逐格 owner-scoped PUT 校验 graded/result/分数范围，使用 workflow revision/CAS 和完全重试幂等；教师覆盖层保留 AI 原始分/理由，并通过统一结果模型影响总览和后续统计。本轮连续工作区不新增后端 API。
- 工程证据：R02 + 问题来源合并回归 `25 passed, 1 skipped`；visible-scope audit、lint、TypeScript 与 Vite production build（`484 modules`）通过。
- 浏览器证据：历史证据 `R02-review-detail-desktop.png`、`R02-review-detail-mobile.png` 保留；2026-07-27 最新证据 `review-continuous-workspace-20260727.png` 验证当前学生全题连续显示、题号栏、学生回顶、中文筛选与真实保存。匿名 fixture，未调用 provider 或修改用户任务。
...
- [~] R02-03 题目导航由搜索、左侧独立滚动目录及每题上下题按钮共同提供；工程完成：三者与正文当前题联动，不再切换另一套 route 视觉。
...
- [x] R02-10 用户已取消 `全部题目 / 单题` 模式；当前学生全部筛选题目始终连续展示，精确题号只作为初始锚点。
- [x] R02-11 从复核矩阵进入时精确定位对应题目，同时保留该学生其他题目、左栏和统一键盘操作，不再复制一套单题页面。
...
- [~] A06-01 顶部显示及格率、平均数、中位数、最高/最低分和样本数。工程完成：六项 Figma 14 式独立指标卡均由当前筛选范围内的正式结果确定性计算。
- [~] A06-02 默认图表至少覆盖分数分布、逐题得分率/复核量、学生 x 题目热力图、置信度/分歧散点等不同用途。工程完成：另含互斥的及格构成图，共五类真实图表。
- [~] A06-03 每张图说明指标、样本、筛选范围、数据版本和明细入口。工程完成：范围写入 URL，图表元信息与题目/学生分流同步更新。
- [~] A06-04 支持自然语言请求更多图表，生成结果走安全 schema。工程完成边界：显式提交才调用一次既有 chart API；后端与前端双重白名单/数量上限已接入，schema 定向测试通过；浏览器未消费真实 provider。
- [~] A06-05 支持保存/删除自定义图表、恢复默认图表和导出 PNG/PDF。工程完成边界：实际 SVG→PNG、浏览器打印/存 PDF 和本次访问内存保存/删除已完成；跨刷新持久化及版本化 artifact 仍待后端。
- [~] A06-06 使用真实图表库；禁止把 scatter、box 等伪装成柱状缩略图。工程完成：`recharts@3.10.0` 渲染 bar/pie/composed/scatter，box 使用真实四分位箱线 SVG；图表页独立懒加载。

### 8.7 A-06 可视化分析阶段工程记录（2026-07-24）

- 详细决定与验收矩阵：`docs/20260724/A06_VISUAL_ANALYSIS_STAGE_DECISION_AND_ACCEPTANCE_CN.md`。
- 默认图表全部消费不可变正式结果，零 provider；只有教师主动确认自然语言请求后才发起一次模型调用。
- `npm run lint`、TypeScript、Vite `933 modules` build、`git diff --check` 与后端 analytics/schema 定向 `3 passed` 通过。
- 浏览器证据：`A06-visual-analysis-desktop.png`（1440×900）与 `A06-visual-analysis-mobile.png`（390×844）；两端无页面级横向溢出，5 类默认图表和 12 个真实 SVG 均已渲染。
- 当前状态：代码、统计口径、真实图表、范围切换、导出边界、工程和浏览器验收完成，等待用户视觉确认，故保持 `[~]`。下一阶段 A07 只做报告与下载。

### A-07 报告与下载 `/tasks/:id/results/reports`

- [~] A07-01 成绩表、学情报告、发布版标答、LaTeX、Markdown 等产物显示状态、版本和生成时间。工程完成：CSV、事实学情 MD、标答 MD/TEX、正式结果 JSON 五项均有真实字节、大小、SHA-256、版本和时间。
- [~] A07-02 支持单项下载、下载全部和按最新 final result 重新生成。工程完成：后端单项与 ZIP 下载、最新版本 CAS 生成和指纹幂等已接入；相同版本返回 `already_done`，不制造重复副本。
- [~] A07-03 旧版本清楚标注，不允许悄悄下载过期报告。工程完成：版本索引明确区分 current/historical/stale/not_generated；历史区默认折叠、下载二次确认，后端 URL 强制显式版本且当前缺文件不回退旧版。

### 8.8 A-07 报告与下载阶段工程记录（2026-07-24）

- 详细决定与验收矩阵：`docs/20260724/A07_REPORTS_DOWNLOADS_STAGE_DECISION_AND_ACCEPTANCE_CN.md`。
- 后端定向/合并回归 `24 passed`；生成、幂等、教师最终分 CSV、真实 ZIP、五项文件、owner 隔离和 v1/v2 历史边界均已覆盖。
- 前端 81 文件 visible-scope/lint/TypeScript 与 Vite `939 modules` build 通过；A07 为 `15.02 kB` 独立懒加载 chunk。
- 浏览器证据：`A07-reports-downloads-desktop.png`（1440×900）与 `A07-reports-downloads-mobile.png`（390×844）；生成前后、单项/ZIP、切页回顶与无页面级横向溢出通过，匿名 fixture、零 provider。
- 当前状态：代码、版本/下载合同、工程和浏览器验收完成，等待用户视觉确认，故保持 `[~]`。下一步先读取官方额度并选择剩余 QA 阶段。

---

## 9. 测试、视觉 QA 与逐阶段验收

### 9.1 R6-01 / R6-02 页面专注度与信息密度工程记录（2026-07-24）

- [~] R6-01 每个 canonical 页面通过“一页一个任务”工程审计；上传、进度、矩阵总览、详情、批量处理、批改、复核和正式分析均为独立 route。长滚动只保留给用户选择的全部题目视图及真实列表/报告，等待用户逐页视觉确认。
- [~] R6-02 页面长度、主操作数和卡片层级工程审计完成；总览不再内嵌详情，可见页面无遗留 `Card` / `WorkflowSection` 容器，开发者待接术语命中为 0，并加入 lint 防回归。等待用户逐页视觉确认。
- 详细记录：`docs/20260724/R6_01_02_PAGE_FOCUS_AND_DENSITY_STAGE_DECISION_AND_ACCEPTANCE_CN.md`。
- 本轮复用既有 `1440×900` / `390×844` 证据，不重新截图；R5-07 已补 1280px 风险检查并修复工作台溢出。

### 9.2 R6-03 / R6-04 视觉证据与响应式回流记录（2026-07-24）

- [~] R6-03 已完成 17 张 Figma frame 到 canonical route 和现有截图的逐项证据清单；本轮新增 G03、Q01、Q08 `1440×900`，不重复已有页面。Q07 与 R02 1440 保留用户抽查，故不勾成 `[x]`。
- [~] R6-04 既有 390px 双尺寸证据、R5-07 的 1280 核心 route 巡检及本轮 Q01/Q08 1280 风险检查通过；430px 不存在新增断点风险，不机械重复截图。原生 200% 与读屏待用户终验。
- 本地精确视口 Chromium 最终 `0 errors / 0 warnings`；验收草稿已删除，零 provider。
- 详细记录：`docs/20260724/R6_03_04_VISUAL_EVIDENCE_AND_REFLOW_STAGE_DECISION_AND_ACCEPTANCE_CN.md`。

### 9.3 R6-05 / R6-06 / R6-07 最终工程回归（2026-07-24）

- [~] R6-05 题目和作答两条流程的逐页浏览器证据、保存闭环及全量合同回归完成；当前 0/0 模型，未消费真实 provider 重放端到端识别，故保持 `[~]`。
- [~] R6-06 不存在任务/结果、0 模型 BYOK、Q08 前置门禁及后端 `429/408/413/503`、quota、timeout、retry、CAS、幂等检查完成；真实供应商成功/限流仍待教师低成本抽查。
...
- [x] QA-02 最终 production build 通过；`934 modules transformed`。
- [~] QA-03 题目、作答、批改、复核、正式结果的分段浏览器闭环和 235 项后端全量合同通过；真实 provider 端到端重放待测试 key。
- [x] QA-04 学期边界、标签去重、课程为空、智能匹配歧义测试。完成：2026-07-27；历史/新建任务合同覆盖月份边界、空课程、owner-scoped exact/related、规范化与并发去重、歧义候选、强制新建和跨 owner 拒绝。
...
### 9.5 Q01 启动条件恢复回归（2026-07-27）

- [x] Q01-BUG-01 配置 BYOK 往返后，空白的可选标答/评分标准/测试资料行不再错误禁用统一准备按钮；启动仅预检实际选择过的来源。
- [x] Q01-BUG-02 没有题目文件或资料库题目来源时，主按钮保留可点击提示态；悬浮给出缺口，点击弹窗给出“上传题目”恢复入口，不再静默灰掉。
- [x] Q01-BUG-03 题目与 BYOK 使用同一可解释门禁，可同时列出多个缺口；真正的原生禁用仅保留给请求进行中、模型配置读取中和读取失败。
- [x] Q01-BUG-04 Safari 原任务 1/1 模型 + 已有题目 + 空可选资料行，以及隔离浏览器 1/1 模型 + 无题目两条回归均通过；未触发 provider，控制台 `0 errors / 0 warnings`。
- [x] Q01-BUG-05 前端 visible-scope audit（69 文件）、TypeScript 和 Vite production build（929 modules）通过。

### 9.6 Q03 / S01 / S05 工作流与交互纠偏（2026-07-27）

- [x] Q03-BUG-01 0 风险仍显示全部题目资料矩阵；三道 fixture 题分别显示题目“已识别”、标答/评分标准“已生成”，不再用大空态隐藏正常项。
- [x] Q03-BUG-02 完整审核首次进入回到页面顶部；精确题目深链将题卡标题定位到全局顶栏下方，左侧题号栏独立滚动且同步高亮。
- [x] FLOW-BUG-01 八步命名纠正为“上传作答 / 校对作答”，任务持久状态推导最远可达步骤；`submissions_ready` 返回 Q03 后步骤 4、5 仍可点击，步骤 6 仍禁用。
- [x] S01-BUG-01 文件选择和识别设置合并为同一路由两个短阶段；保存任务级设置后直接开始作答识别，C01 不再占顶层编号。
...
- [x] QA-04：学期边界、标签去重、课程为空和智能匹配歧义的组合合同已由 2026-07-27 全量回归确认。
...
- [x] FLOW-BUG-02 第 8 步完成态可显式回看第 5“校对作答”、第 6“执行批改”和第 7“复核批改”；各页统一使用最远可达阶段，不再由旧状态守卫强制跳回 `/results`。
- [x] S05-BUG-03 已进入后续阶段的作答详情保留连续题目、独立滚动目录和筛选，但身份/作答为只读；底部主按钮进入第 6 步，返回作答矩阵为次按钮。
- [x] C02/C03-HISTORY-01 C02 展示历史配置快照且不可重复启动；C03 展示 `100%` 已完成队列快照并可进入复核批改。
...
- [x] R02-VISUAL-01 保持 Figma 15 的内容层级，并按最终纠偏使用学生导航、题目搜索、左侧题号目录和连续题卡；SmarTAI/教师双列与评分标准均在各题内部完整显示，无页面级横向溢出。
...
- [x] R7-ERROR-01 API 客户端兼容字符串/对象错误详情，并提取稳定错误码与真实 `Retry-After`；页面不再只显示笼统的 HTTP 文案。
- [x] R7-ERROR-02 建立共享错误分类器与 Figma 17 单焦点恢复卡；题目、作答、批改、正式结果、可视化和报告不再各自维护不同错误视觉。
- [x] R7-BYOK-01 缺失模型统一进入 `/settings/byok` 并保留安全 `returnTo`；返回后保留已选资料和页面输入。
- [x] R7-FILE-01 文件不可解析、过大、空文件、来源过期/变化分别提供重传或重选动作；未实现 OCR 时不声称扫描件可处理。
- [x] R7-RATE-01 只有后端提供真实等待值才显示重试时间；前端不自行倒计时或自动消耗模型额度。
- [x] R7-SAFE-01 技术详情默认折叠并使用字段白名单；不展示 key、答案/文件正文、原始 provider 异常或日志堆栈。
- [x] R7-QA-01 69 文件 visible-scope/lint/TypeScript、Vite production build（`934 modules transformed`）与报告/可视化/题目上传浏览器巡检通过；控制台 `0 errors / 0 warnings`，未调用真实 provider。
- 详细记录：`docs/20260727/R7_FIGMA17_RECOVERABLE_ERRORS_STAGE_CN.md`。

### 9.12 R8 前端工程收口与最终确定性回归（2026-07-27）

- [x] R8-ROUTE-01 Figma 01–17 与讨论补充的教师主流程均有 canonical 页面；五个正式结果 route 在本地教师 fixture 中显示同一顶部大导航且无加载失败。
- [x] R8-DETAIL-01 A03/A05 均保留搜索、题号目录和上一题/下一题，A05 额外保留独立学生切换；没有恢复两套详情 URL 或局部结果侧栏。
- [x] R8-ERROR-01 不存在任务的正式结果 route 实际渲染共享恢复状态、折叠技术信息和安全返回动作，不显示秘密或原始日志。
- [x] R8-QA-01 前端 69 文件 lint/TypeScript、Vite `934 modules` build、`git diff --check` 与后端全量 `235 passed, 1 skipped` 通过。
- [x] R8-QA-02 QA-04 学期/课程/标签/匹配边界完成；QA-06、QA-07、QA-08 的已完成工程范围与需真实 provider/专门 fixture/持久化的边界已经分开记录。
- [~] R8-SIGNOFF-01 工程侧没有尚未搭建的主流程页面；用户逐页视觉签收、原生 200%/读屏、真实 provider、OCR 和持久化仍不能由自动检查代替。
- 详细记录：`docs/20260727/R8_FINAL_FRONTEND_ENGINEERING_CLOSEOUT_STAGE_CN.md`。

### 9.13 R02 连续复核与历史页版心一致性纠偏（2026-07-27）

- [x] R02-CONTINUOUS-01 删除“单题 / 全部”页面模式；精确题号只负责首次定位，当前学生的全部筛选题目始终连续渲染。
- [x] R02-CONTINUOUS-02 复用统一 `ResultQuestionSidebar`；题号栏独立滚动，搜索、题号点击、正文滚动、题卡按钮与键盘上下键同步当前题。
- [x] R02-CONTINUOUS-03 学生切换使用左右键、相邻按钮或直选，并回到页面顶部；题目筛选保留当前学生且兼容中文 composition。
- [x] R02-RESULT-01 “SmarTAI 批改结果”不再截断理由；教师得分置于右上角 `x / x`，只有得分可在该处编辑，补充评语使用下方独立字段。
- [x] R02-SAVE-01 真实分数与评语保存、确认状态、AI 原始结果保留继续复用现有 owner、workflow revision/CAS 和幂等合同；无新增后端 API。
- [x] G02-WIDTH-01 历史页筛选、结果计数、表头/列表/空态和分页统一继承页面最大版心，不再由列表与分页写死 `1080px` 造成右边缘参差。
- [x] G02-WIDTH-02 桌面 `1280px` 实测筛选外框、计数、列表和分页均为 `x=32 / width=1216 / right=1248`，页面 `scrollWidth=1280`；控制台 `0 errors / 0 warnings`。
- [x] LAYOUT-AUDIT-01 已检查工作台、课程资料库、新建任务、设置、BYOK 与主要任务页；未发现其他同页兄弟主白框因固定宽度产生的同类错位。`800/900/940/1100px` 的单一居中表单或单焦点状态卡保留为页面职责约束，不盲目拉满。
- 浏览器证据：`review-continuous-workspace-20260727.png`、`history-unified-width-20260727.png`；均为匿名隔离 fixture，未调用真实 provider。
- 详细记录：`docs/20260727/R02_CONTINUOUS_REVIEW_AND_LAYOUT_ALIGNMENT_STAGE_CN.md`。

### 9.14 S05 / R01 矩阵与待处理队列统一（2026-07-27）

- [x] MATRIX-LAYOUT-01 S05 与 R01 共用 `MatrixQueueWorkspace`；桌面为弹性矩阵 + `280px` 队列、`16px` 间距，矩阵与队列同高且各自滚动。
- [x] MATRIX-COLUMN-01 两页均显式显示“学号 / 姓名 / 题目 / 操作”，每行末尾都有“查看”；题目列收敛为 `60px`，约 10 道题仍可紧凑展示，更多题只在矩阵内部横向滚动。
- [x] MATRIX-STATE-01 状态格占满对应单元格；绿色对钩、蓝色圆圈对钩、橙色警告、红色错误和天蓝评语分别表示正常、已复核、需关注、错误与已有批注，完整含义保留在 `aria-label/title`。
- [x] MATRIX-QUEUE-01 S05 待处理队列与 R01 待复核队列使用同一窄列节奏，列出全部真实问题并直达对应学生/题目；没有问题时只显示单一空态。
- [x] R01-ACTION-01 “查看批改详情”位于“确认复核完成”左侧，两者统一放在矩阵工作区底部右侧；已完成任务保持只读并显示“查看最终结果”。
- [x] FLOW-COPY-01 第 7 步唯一可见命名改为“复核批改 / Review Grading”，第 8 步继续为“结果分析 / Results Analysis”。
- [x] MATRIX-QA-01 真实 `T_39aca252a1` 的 S05/R01 均完成浏览器验收；S05 4 行均有“查看”，12 个状态格均有可访问名称，两页无页面级横向溢出，干净标签页控制台均为 `0 errors / 0 warnings`。前端 lint、TypeScript 和 Vite production build（`937 modules transformed`）通过。
