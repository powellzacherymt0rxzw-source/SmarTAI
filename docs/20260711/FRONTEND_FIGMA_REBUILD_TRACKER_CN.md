...
>
> 当前阶段：G-03 New Task 已于 2026-07-15 完成实现、工程验收、1440×900/390×844 截图和真实创建流程；等待用户阶段验收，未开始 Q-01。
...
- [~] BE-META-06 提供课程/标签分页搜索、完全匹配、模糊匹配、语义候选和新建前二次查重。工程进展：2026-07-15；owner-scoped 分页搜索、NFKC/casefold 精确匹配、确定性词法相近候选、结构化 409 和“仍然新建”已完成；拼音与真实语义模型匹配仍未完成，缺安全限额和用户级 BYOK 隔离时不会伪装为已支持。
...
- [~] G03-01 任务名称必填。工程完成：2026-07-15；浏览器 required 校验与后端空白 422 双层约束，等待用户阶段验收。
- [~] G03-02 学期使用固定选项，默认当前学期，范围遵循确认后的算法。工程完成：2026-07-15；2026 年 7 月默认夏季并只向后多显示 2026-2027 秋季，等待用户阶段验收。
- [~] G03-03 课程可选，使用智能可搜索/可创建 combobox。工程完成：2026-07-15；真实 owner-scoped API、180ms 防抖、键盘导航、选择/移除/显式新建已接入，等待用户阶段验收。
- [~] G03-04 删除教学班/分组字段。工程完成：2026-07-15；页面和 task payload 均没有这两个固定字段，等待用户阶段验收。
- [~] G03-05 提供可选标签，教学班等信息由标签表达。工程完成：2026-07-15；真实多选标签和显式新建已接入，等待用户阶段验收。
- [~] G03-06 课程填写后自动增加课程系统标签。工程完成：2026-07-15；通过 `course_id` 关系在 History 中展示为系统标签，不复制同名用户标签，等待用户阶段验收。
- [~] G03-07 标签完全/模糊/语义匹配和新建确认可解释。工程进展：2026-07-15；完全匹配、确定性词法相近、新建及相似项二次确认已完成；拼音和真实语义模型匹配仍未完成，因此本项不能标 `[x]`。
- [~] G03-08 页面只保留创建任务所需字段；专家/OCR 可继承并以摘要/高级设置出现。工程完成：2026-07-15；只保留当前模型配置摘要和 BYOK 入口，不显示 OCR、专家选择或高级设置，等待用户阶段验收。
- [~] G03-09 创建后进入添加题目页，不在当前页继续堆上传流程。工程完成：2026-07-15；真实流程进入 `/tasks/:id/upload/problems`，Figma 04/Q-01 未混入本页，等待用户阶段验收。

### 5.4 G-03 阶段实现与工程验收记录

- 状态：实现、工程验收与真实浏览器创建流程完成；等待用户阶段验收，尚未标记 `[x]`，也不进入 Q-01。
...
- 待用户人工检查：中英文文案观感，以及极端长课程名、多标签同时选中时是否符合个人阅读偏好；工程上已保证不重叠。若通过，再开始 Q-01/Figma 04；若不通过，只在本页收敛修改。
- 数据边界：Task/Course/Tag/幂等记录仍为进程内存；拼音/真实语义相似检索、PostgreSQL 持久化和跨资源事务未实现；Q-01 上传页仍是旧实现。
