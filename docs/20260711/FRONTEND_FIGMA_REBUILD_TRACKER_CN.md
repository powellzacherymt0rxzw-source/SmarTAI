...
>
> 当前阶段：G07 当前任务信息与 BYOK 门禁纠正已完成工程和真实浏览器验收，等待用户视觉确认。新增 `/tasks/:taskId/edit` 复用 Figma 03 表单；Q01 可保留来源草稿返回编辑；新标签创建后关闭弹层；0/0 模型时灰态 CTA 可点击查看 BYOK 原因。下一阶段按 2026-07-26 最新覆盖先重做 Q01 四资料位与一次识别编排，再重做风险优先 Q03；不能继续完善已经被覆盖的 Q08/Q09 独立入口。
...
- [ ] Q01-09 将来源工作区扩为题目、标答、评分标准、编程题测试样例四个资料位；题目必选，其余可选；支持左右切换和连续查看。
- [ ] Q01-10 每份资料独立选择上传/课程资料库、已按题整理/从原文提取及提取说明；不得继续让多类资料共用一个来源设置。后端四类均支持多来源（题目至少一份）；来源冲突保留候选和风险，不按文件顺序静默覆盖。
- [ ] Q01-11 主动作改为“识别并准备题目资料”，一个编排处理所有上传来源并生成未上传的标答/评分标准；只含最终答案时补全过程，输出答案步骤与评分步骤对应关系。
- [ ] Q01-12 后端为每题保存题干/标答/评分标准的来源、置信度、文件冲突与解析异常；正常生成完成不进入缺失补录路径。
- [~] Q01-13 0/0 模型门禁可解释。工程与浏览器验收完成：2026-07-26；灰态 CTA 悬浮解释、点击对话框和携带草稿的 BYOK 入口已接入，等待用户视觉确认。

2026-07-26 最新覆盖已重新打开 Q01 页面定义。Q01-01 至 Q01-08 记录的是单题目来源版本的真实能力和安全合同，可复用但不代表新四资料位流程已完成；下一阶段必须先交付 Q01-09 至 Q01-12，再修改 Q03。
...
- [~] Q03 题目准备总览改为风险优先矩阵。旧覆盖率大卡、独占搜索行和首屏矩阵曾于 2026-07-20 归档；2026-07-26 最新要求重新打开本项：正常识别/生成完成的数据不显示“缺失”，矩阵只突出低置信、AI 与原文件冲突、解析异常和确有不适用；删除“批量导入资料/AI 补全缺失项”入口，并补 composition 安全搜索、表头排序与多选列筛选。
- [~] Q04–Q07 题目知识包连续审核页。现有字段保存合同可迁移，但最终设计只有一个连续长页：左侧可独立滚动题号目录并随正文同步高亮，右侧每题依次完整展示题干、标答、步骤对应评分标准，只有编程题才出现测试样例；各部分独立编辑。取消“全部/单题”和资料维度标签；前后题按钮只滚动定位，底部一次确认全部。浏览态渲染 LaTeX，修改态显示源码。现有分裂详情页不再视为最终完成。
