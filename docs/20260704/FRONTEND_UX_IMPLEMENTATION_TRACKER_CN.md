...
- [ ] P1-03 建立单栏页面 section / toolbar / table primitives。完成：
- [ ] P1-04 建立 inline help / tooltip 轻量组件。完成：
...
- [ ] P2-03 实现或增强 TaskStageGate，未到阶段显示当前阶段与跳转动作。完成：
- [ ] P2-04 重新上传/覆盖危险动作给出明确确认入口。完成：
...
- [ ] P4-01 新增或重构题目准备总览：覆盖率、资料配置状态、智能筛选入口。完成：
...
- [ ] P5-01 重构作答校对总览：学生 x 题目状态表、双维度筛选。完成：
- [ ] P5-02 重构批改前确认：专家组合、资料范围、评分策略、风险提醒。完成：
...
- [ ] P6-01 新增可恢复错误面板：BYOK、API 超量、网络超时、解析失败。完成：
- [ ] P6-02 关键按钮接入 BYOK 缺失禁用与跳转提示。完成：
- [x] P6-03 全量术语检查：页面不再出现不应出现的“材料”。完成：2026-07-04 09:59；范围为 `frontend/app/src` 用户可见中文。
- [ ] P6-04 全量视觉检查：彩色 pill 占比降低，不显乱。完成：
- [x] P6-05 运行 `npm run typecheck`。完成：2026-07-04 09:59。
- [x] P6-06 运行 `npm run build`。完成：2026-07-04 09:59。
