...
- 后端尚无持久化 `review_confirmed / finalized` 生命周期，因此没有开放 Figma 中“确认复核完成并生成分析/导出”的伪按钮；主操作改为真实可用的“开始优先复核”。正式确认合同留给 A00 阶段。
...
- A00 尚需新增 owner-scoped、revision/CAS、幂等的复核确认与分析生成状态合同；在此之前不得将任务标记为正式完成。
