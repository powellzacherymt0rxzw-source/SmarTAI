...
本阶段完成 S05“单份作答校对”：教师只处理一名学生的一道题，学生与题目两个维度独立查找和切换，并保存真实识别文本与识别提示。
...
2. 题目导航独占第二行：上一题、当前题、智能查找、下一题和 `↑ / ↓` 提示。
3. 学生识别文本为最显眼的全宽编辑区。
4. 下方仅两块并列信息：真实题干上下文；真实识别状态与提示。
5. 原始来源使用单一全宽短条；没有可靠页码、区域坐标或置信度时明确显示缺失边界。
6. 保存区位于首屏底部；题目导航在页面底部再提供一次，短滚动即可到达。
