...
| 单页图表/下载布局 | 扩展成五个独立入口：总览、题目分析、学生分析、可视化分析、报告与下载；2026-07-27 最终覆盖为顶部等宽大入口 |
| 完成与下载 CTA | 只有真实 artifact 存在后才开放；A00 对未生成内容只显示事实空态，没有伪下载按钮 |

2026-07-27 用户最终覆盖：结果工作区导航位于版本横幅下方，桌面五项等宽排列，窄屏只在导航容器内横向滚动；不再占用内容左侧，也不再折叠为语义不同的选择器。释放的左侧空间用于题目/学生详情共用题号目录。流程条仍自动定位“复核 → 完成”，主内容不发生页面级横向滚动。
