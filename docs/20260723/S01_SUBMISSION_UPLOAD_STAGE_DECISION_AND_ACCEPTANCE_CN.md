...
本阶段最初只完成 S01“添加学生作答”。2026-07-27 按用户纠正将“选择作答文件”和“识别/批改设置”合并为同一 canonical route 的两个短阶段；识别进度、校对矩阵和连续作答审核仍保持独立页面，不恢复旧长页。
...
- 选择作答文件后进入同一路由内的“作答识别设置”，在这里保存任务级设置并立即开始识别，不再要求教师先绕到独立 C01 再返回。
- 作答识别默认使用刚保存的 primary provider；不在上传页另放一套容易冲突的 provider 下拉框。
...
- 交互：文件名、名单、人工复核三种模式均可切换；名单模式真实出现名单文件选择器；切回文件名模式状态正确。2026-07-27 起无文件时 CTA 不再静默禁用，而是可点击并显示“请先选择学生作答文件”。
...
## 8. 2026-07-27 上传作答内部两阶段纠偏

- 顶层八步中第 4 步固定为“上传作答”，不再另设“批改设置”编号。
- `/tasks/:id/submissions/upload` 内部先选择文件和身份匹配，再切换到识别设置；文件对象在同一 SPA 会话按任务暂存，从 BYOK 返回也能恢复。
- 第二阶段复用现有任务级设置合同，但使用当前 Figma 平面卡片、克制边框和主按钮“保存设置并开始识别作答”；保存完成后直接调用现有 `parse_submissions`。
- 没有文件时 CTA 保持可点击并显示明确行内错误；进行中、任务不可用或批改锁定等真实状态继续阻断。
- 本轮不需要新增后端 API；设置保存与作答解析仍分别使用已有 owner/version/provider 校验。
