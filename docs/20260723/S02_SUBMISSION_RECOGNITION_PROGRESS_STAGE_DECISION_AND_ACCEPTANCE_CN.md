...
- S03 尚未实现，因此 `submissions_ready` 暂时进入兼容旧工作区 `/tasks/:id/upload/submissions`；下一阶段必须把该唯一过渡目的地替换为 canonical `/tasks/:id/submissions`。
...
S02 已满足用户视觉确认条件，但只有用户明确确认后才把 `[~]` 改为 `[x]`。下一阶段 S03 应继续遵守：
