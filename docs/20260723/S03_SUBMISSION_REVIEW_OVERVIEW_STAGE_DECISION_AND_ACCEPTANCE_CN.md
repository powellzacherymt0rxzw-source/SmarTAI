...
- 后续更新：S04 已于 2026-07-23 完成，S05 已于 2026-07-24 完成；当前下一阶段为 C02 批改前确认。

S03 不编辑整份作答、不展示批改配置、不启动批改，也不把所有学生答案铺在矩阵下方。阶段交付时矩阵单元格先进入对应学生 S04 并携带 `?question=:qId`，不会指向不存在的页面；后续 S05 已建成，单元格现直接进入精确单题校对 route 并保留矩阵筛选。
...
点击学号或“查看”进入 S04；点击题目单元格现进入 S05 精确单题校对页。学生和题目两个维度不会互相清空，返回时通过 `returnParams` 恢复 S03 的查询、状态和排序。
...
2. 后续更新：S05 已负责学生 × 题目的聚焦校对、双维搜索和键盘方向切换；当前单元格已直达 S05。
